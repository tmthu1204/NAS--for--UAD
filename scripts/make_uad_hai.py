...
def infer_default_out_dir(source_dir: Path, target_dir: Path, shift_level: str) -> Path:
    ...
    root = source_dir.parent.parent / "hai_experiments"
    if same_entity:
...
def build_args(parsed):
    source_dir = Path(parsed.entity_dir)
    target_dir = Path(parsed.target_entity_dir) if parsed.target_entity_dir else source_dir
    ...
    return SimpleNamespace(
        ...
        target_machine_dir=str(target_dir) if parsed.target_entity_dir else None,
...
def build_arg_parser():
    ...
    ap.add_argument(
        "--entity_dir",
        required=True,
        help="Source HAI cached entity directory, e.g. data/hai/hai21_03-train1",
    )
    ap.add_argument(
        "--target_entity_dir",
        default=None,
        help="Optional target HAI cached entity directory, e.g. data/hai/hai21_03-test2",
    )
