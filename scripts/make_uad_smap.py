...
def infer_default_out_dir(source_dir: Path, target_dir: Path, shift_level: str) -> Path:
    ...
    root = source_dir.parent.parent / "smap_experiments"
    if same_entity:
...
def build_arg_parser():
    ...
    ap.add_argument(
        "--channel_dir",
        required=True,
        help="Source SMAP channel directory, e.g. data/smap/A-1",
    )
    ap.add_argument(
        "--target_channel_dir",
        default=None,
        help="Optional target SMAP channel directory for cross-entity covariate shift. If omitted, use the same channel.",
    )
