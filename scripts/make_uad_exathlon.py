...
from scripts.make_uad_smd import (
    DEFAULT_ALLOW_SINGLE_CLASS_EVAL,
    DEFAULT_GUARD,
...
def infer_default_out_dir(source_dir: Path, target_dir: Path, shift_level: str) -> Path:
    ...
    root = source_dir.parent.parent / "exathlon_experiments"
    if same_entity:
...
def build_args(parsed):
    source_dir = Path(parsed.trace_dir)
    target_dir = Path(parsed.target_trace_dir) if parsed.target_trace_dir else source_dir
    ...
    return SimpleNamespace(
        ...
        target_machine_dir=str(target_dir) if parsed.target_trace_dir else None,
...
def build_arg_parser():
    ...
    ap.add_argument(
        "--trace_dir",
        required=True,
        help="Source Exathlon cached trace directory, e.g. data/exathlon/app1-0-1000000-14",
    )
    ap.add_argument(
        "--target_trace_dir",
        default=None,
        help="Optional target Exathlon cached trace directory. If omitted, use the same trace.",
    )
    ap.add_argument("--out_dir", default=None)

    ap.add_argument("--split_mode", choices=["search", "fixed"], default=DEFAULT_SPLIT_MODE)
    ap.add_argument("--shift_level", choices=["auto", "hard", "medium", "mild"], default=DEFAULT_SHIFT_LEVEL)
    ...
    if DEFAULT_ALLOW_SINGLE_CLASS_EVAL:
        ap.add_argument("--allow_single_class_eval", action="store_false")
    else:
