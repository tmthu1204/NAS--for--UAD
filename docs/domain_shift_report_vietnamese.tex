...
\begin{center}
    ...
    \textbf{Mục tiêu: Viết lại báo cáo theo khung Vấn đề, Mục đích, Related Work, Method, Công thức; và sử dụng TS-TCC làm encoder cho JMMD và CORAL.}
\end{center}
...
\midrule
\endhead
TS-TCC \cite{tstcc} & ICLR 2022 & Học biểu diễn temporal và contextual tự giám sát cho time series. & TS-TCC cung cấp encoder contextual cần thiết để đo domain shift trong latent time-series space. \\
JAN \cite{jan} & ICML 2017 & Căn chỉnh \emph{joint distributions} của nhiều tầng domain-specific bằng JMMD. & JMMD là lựa chọn mạnh hơn plain MMD vì không chỉ nhìn một marginal distribution. \\
...
\subsection{Vì sao dùng TS-TCC cho contextual representations}

Trong multivariate time series anomaly detection, domain shift nên được đo trong latent space với temporal context, không nên đo trên raw signals. TS-TCC \cite{tstcc} học biểu diễn bằng cách contrast temporal và contextual views, làm nó lý tưởng để capture hierarchical features qua nhiều tầng. Điều này phù hợp với nhu cầu multi-layer features trong JMMD và final embeddings trong CORAL.

\subsection{Vì sao cần 2 view: Representation và Dependency Shift}

Domain shift trong transfer learning bao gồm nhiều khía cạnh, bao gồm representation shift (mismatch trong phân phối features) và dependency shift (mismatch trong phụ thuộc features hoặc correlations). Representation shift đo cách features từ các domain khác nhau khác nhau trong marginal distributions, quan trọng cho deep models nơi layers capture hierarchical abstractions. Dependency shift, mặt khác, bắt differences trong second-order statistics, như covariances, phản ánh cách features tương tác trong domains. Cả hai đều cần thiết vì domain shift có thể manifest ở cả feature-level mismatches (representation) và relational mismatches (dependency), và bỏ qua một trong hai dẫn đến incomplete adaptation assessment. Cách tiếp cận dual-view này được hỗ trợ bởi surveys về transfer learning, nhấn mạnh rằng comprehensive domain shift measurement yêu cầu address cả distributional và relational differences \cite{pan2010survey}.

\subsection{Lập trường chốt cho báo cáo}

Từ các paper trên, hướng viết chặt nhất là:
\begin{enumerate}[label=\arabic*.]
    \item dùng TS-TCC làm contextual encoder;
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item dùng contextual representation theo tinh thần TS-TCC \cite{tstcc};
...
Tại sao sử dụng JMMD, CORAL và DANN để tính domain shift, và tại sao không sử dụng các view còn lại?

JMMD được sử dụng vì nó đo representation shift qua nhiều tầng, bắt được mismatch phân cấp trong temporal context, quan trọng cho multivariate time series nơi shift xảy ra ở các mức trừu tượng khác nhau. CORAL được sử dụng vì đo dependency shift qua chênh lệch covariance, bổ sung JMMD bằng cách tập trung vào second-order statistics phản ánh coupling giữa sensors. DANN được sử dụng như sanity check cho domain separability trong latent space, đảm bảo encoder không dễ dàng phân biệt domains.

Các view khác như plain MMD trên một tầng không được dùng vì không bắt được multi-layer shifts, làm chúng kém nghiêm túc cho deep representations. Raw summary statistics hoặc mean-std features bị tránh vì bỏ qua temporal context và latent geometry. Domain classifiers ngoài DANN không dùng làm metric chính do không ổn định từ phụ thuộc tối ưu.
